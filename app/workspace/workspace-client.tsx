...
export function WorkspaceClient({ userId, userName, products }: { userId: string; userName: string; products: ProductLine[] }) {
  ...
  return <div className="flex flex-col gap-16">
    <header className="flex flex-col gap-5 border-b-2 border-rule pb-10"><div className="flex items-center justify-between gap-4"><p className="eyebrow">个人工作台 · {userName}</p><LogoutButton userId={userId} /></div><h1 className="headline max-w-3xl text-4xl sm:text-5xl">从这里，进入牧之的 AI 产品系统。</h1><p className="max-w-2xl text-lg leading-8 text-ink/75">先用已经可用的产品。其余工程正在搭建。</p></header>
    ...
    <section className="rule-top flex flex-col gap-6 pt-12"><h2 className="headline text-2xl">完整产品矩阵</h2><p className="max-w-2xl text-ink/70">五个字母是牧之名字的拆解。每条产品线独立运行，沿着同一个入口逐步展开。</p><Link href="/projects" className="font-mono text-sm text-accent-readable underline underline-offset-4">查看全部项目 →</Link></section>
  </div>;
}
